...
describe('useStoryCharacters', () => {
  ...
  it('closes after confirmed discard of unsaved changes', async () => {
    const services = createServices()
  ...
  it('does not delete from invalid states or when confirmation is cancelled', async () => {
    const services = createServices()
    vi.spyOn(window, 'confirm').mockReturnValue(false)
    const { result } = renderHook(() =>
      useStoryCharacters({
  ...
  it('keeps the dialog open when character deletion throws', async () => {
    const services = {
      ...createServices(),
      deleteCharacter: vi.fn(() =>
        Promise.reject(new Error('Could not delete character.')),
      ),
    }
    vi.spyOn(window, 'confirm').mockReturnValue(true)
    const { result } = renderHook(() =>
    ...
    expect(result.current.dialogState.mode).toBe('view')
    expect(result.current.errorMessage).toBe('Could not delete character.')
  })
})
