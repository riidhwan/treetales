...
For pull requests, include a concise description, linked issue when relevant (`Refs #N` or `Closes #N`), verification commands, and screenshots for UI changes. Routine development should use topic branches and PRs into `master`; direct pushes to `master` are for emergencies.

## GitHub Issues & Larger Work

Use GitHub Issues as the durable task queue for non-trivial work. Check for an existing issue before implementation; create or draft one unless the change is tiny. Large work should use a parent issue plus child issues for independently shippable vertical slices. Each child issue must leave `master` buildable, testable, deployable, and safe for normal users.
